...
export const TRIGGERS: Record<string, (s: GameState) => boolean> = {
  ...
  threatWithinOne: (s) => perceivedIds(s).size > 0,
  wounded: (s) => s.stats.wounds > 0,
  infectionInHand: (s) => s.player.hand.some((u) => isInfection(u)),
  infectionNearThreshold: (s) => infectionCount(s) >= infectionThreshold(s.depth) - 1,
  hiveHalf: (s) => s.ship.hive >= Math.ceil(hiveWake(s.depth) / 2),
  motherAwake: (s) => s.ship.motherWoken,
  ...
  nearlyGone: (s) =>
    [...s.player.hand, ...s.player.deck, ...s.player.discard].filter((u) => !isInfection(u)).length <= 3,
};
