...
import type { Action, Depth, RoleId } from '../src/engine/types';
...
describe('golden replays (gate 1.9)', () => {
  ...
  for (const f of list) {
    it(`${f.seed} (${f.role} d${f.depth}, ${f.bot}) still ends in ${f.ending}`, () => {
      const final = replay(initialState(f.seed, f.role, f.depth), f.actions);
  ...
  it('covers every ending across the fixture set', () => {
    const endings = new Set(list.map((f) => f.ending));
